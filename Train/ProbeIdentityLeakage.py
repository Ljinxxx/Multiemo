...
from TrainMultiEMO import TrainMultiEMO
...
def extract_features(
    ...
    for _, data in enumerate(dataloader):
        ...
        speaker_ids_2d = trainer.get_speaker_ids_2d(
            padded_speaker_masks,
            padded_labels_2d
        )
        ...
        adj = trainer.build_dialogue_graph_adj(
            speaker_ids_2d,
            padded_labels_2d
        )
        ...
        fc_feature_list.append(
            fc_outputs.detach().cpu()
        )

        emotion_feature_list.append(
            emotion_feature.detach().cpu()
        )

        identity_feature_list.append(
            identity_feature.detach().cpu()
        )

        speaker_label_list.append(
            speaker_labels.detach().cpu()
        )

        emotion_label_list.append(
            labels.detach().cpu()
        )
