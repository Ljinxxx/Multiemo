...
# ============================================================
# 24G GPU clean disentanglement experiment script
# ============================================================
...
python -m py_compile Model/GraphDisentangle.py || exit 1
python -m py_compile Utils/seed_utils.py || exit 1
python -m py_compile Utils/graph_utils.py || exit 1
python -m py_compile Utils/loss_utils.py || exit 1
python -m py_compile Utils/checkpoint_utils.py || exit 1
python -m py_compile Train/TrainMultiEMO_clean.py || exit 1
python -m py_compile Train/ProbeIdentityLeakage_clean.py || exit 1
...
run_probe () {
  ...
  python Train/ProbeIdentityLeakage_clean.py \
...
run_exp \
  "IEMOCAP_main_disentangle_24G_clean" \
  "Log/IEMOCAP/Disentangle24G/IEMOCAP_main_disentangle_clean_seed${SEED}.put" \
  python Train/TrainMultiEMO_clean.py \
    --dataset 'IEMOCAP' \
...
if [ -f Checkpoints/IEMOCAP/best_disentangle_checkpoint.pt ]; then
  cp Checkpoints/IEMOCAP/best_disentangle_checkpoint.pt \
     Checkpoints/IEMOCAP/best_disentangle_checkpoint_main_24G_clean_seed${SEED}.pt
fi

run_probe \
  "IEMOCAP_identity_leakage_probe_24G_clean" \
  "Checkpoints/IEMOCAP/best_disentangle_checkpoint_main_24G_clean_seed${SEED}.pt" \
  "Log/IEMOCAP/Disentangle24G/IEMOCAP_identity_leakage_probe_clean_seed${SEED}.put" \
  "Log/Summary/IEMOCAP_identity_leakage_probe_24G_clean.txt" \
  "$IEMOCAP_BATCH_SIZE"

run_exp \
  "MELD_main_disentangle_24G_clean" \
  "Log/MELD/Disentangle24G/MELD_main_disentangle_clean_seed${SEED}.put" \
  python Train/TrainMultiEMO_clean.py \
    --dataset 'MELD' \
...
if [ -f Checkpoints/MELD/best_disentangle_checkpoint.pt ]; then
  cp Checkpoints/MELD/best_disentangle_checkpoint.pt \
     Checkpoints/MELD/best_disentangle_checkpoint_main_24G_clean_seed${SEED}.pt
fi

run_probe \
  "MELD_identity_leakage_probe_24G_clean" \
  "Checkpoints/MELD/best_disentangle_checkpoint_main_24G_clean_seed${SEED}.pt" \
  "Log/MELD/Disentangle24G/MELD_identity_leakage_probe_clean_seed${SEED}.put" \
  "Log/Summary/MELD_identity_leakage_probe_24G_clean.txt" \
  "$MELD_BATCH_SIZE"
...
run_iemocap_ablation "IEMOCAP_A_no_graph_24G_clean"              0 0    0    0    0     0
run_iemocap_ablation "IEMOCAP_B_gnn_residual_only_24G_clean"     1 0.1  0    0    0     0
run_iemocap_ablation "IEMOCAP_C_graph_emotion_only_24G_clean"    1 0.1  0.01 0    0     0
run_iemocap_ablation "IEMOCAP_D_light_identity_24G_clean"        1 0.1  0.01 0.01 0     0
run_iemocap_ablation "IEMOCAP_E_identity_adv_24G_clean"          1 0.1  0.01 0.01 0.005 0
run_iemocap_ablation "IEMOCAP_F_full_disentangle_24G_clean"      1 0.1  0.01 0.01 0.005 0.001
...
run_meld_ablation "MELD_A_no_graph_24G_clean"                   0 0     0 0     0     0
run_meld_ablation "MELD_B_identity_only_24G_clean"              1 0     0 0.005 0     0
run_meld_ablation "MELD_C_adv_only_24G_clean"                   1 0     0 0     0.002 0
run_meld_ablation "MELD_D_identity_adv_24G_clean"               1 0     0 0.005 0.002 0
run_meld_ablation "MELD_E_identity_adv_weak_residual_24G_clean" 1 0.002 0 0.005 0.002 0
...
SUMMARY_TXT="Log/Summary/disentangle_probe_and_ablation_24G_clean_summary.txt"
SUMMARY_CSV="Log/Summary/disentangle_probe_and_ablation_24G_clean_summary.csv"

rm -f "$SUMMARY_TXT"
rm -f "$SUMMARY_CSV"

echo "===== 24G Clean Config =====" >> "$SUMMARY_TXT"
...
cat Log/Summary/IEMOCAP_identity_leakage_probe_24G_clean.txt >> "$SUMMARY_TXT" 2>/dev/null || echo "No IEMOCAP probe result found" >> "$SUMMARY_TXT"
echo "" >> "$SUMMARY_TXT"

echo "===== Identity Leakage Probe: MELD =====" >> "$SUMMARY_TXT"
cat Log/Summary/MELD_identity_leakage_probe_24G_clean.txt >> "$SUMMARY_TXT" 2>/dev/null || echo "No MELD probe result found" >> "$SUMMARY_TXT"
echo "" >> "$SUMMARY_TXT"
...
csv_path = "Log/Summary/disentangle_probe_and_ablation_24G_clean_summary.csv"
...
echo "$SUMMARY_TXT"
echo "$SUMMARY_CSV"
echo "All 24G clean disentanglement probe and ablation experiments finished!"
echo "=========================================="
