...
class TrainMultiEMO():
    ...
    def build_dialogue_speaker_temporal_adj(
        ...
        """
        Build graph adjacency for valid utterances in one batch.

        Nodes:
            all valid utterances in current batch

        Edges:
            1. self-loop
            2. same dialogue + same speaker
            3. same dialogue + adjacent utterance
    ...
    def train_or_eval_model_per_epoch(self, dataloader, train=True):
        ...
        for _, data in enumerate(dataloader):
            ...
            #   fc_outputs -> self.model.mlp(fc_outputs)
            #
            # Residual GNN path:
            #   fc_outputs -> GNN -> gnn_delta
            #   refined_fc_outputs = fc_outputs + alpha * gnn_delta
            #   emotion_logits = self.model.mlp(refined_fc_outputs)
            #
            # SWFC still uses original fc_outputs.
            # ========================================================
            if self.use_gnn:
