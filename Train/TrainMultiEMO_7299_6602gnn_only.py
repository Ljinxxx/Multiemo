...
from CrossTaskGNN import CrossTaskGNN
...
class CrossModalContrastiveLoss(nn.Module):
    ...
    def forward(self, text_features, audio_features, visual_features):
        loss_ta = self.pair_contrastive_loss(text_features, audio_features)
        loss_tv = self.pair_contrastive_loss(text_features, visual_features)
        loss_av = self.pair_contrastive_loss(audio_features, visual_features)
...
class TrainMultiEMO():
    ...
    def get_model(self):
        ...
        self.cross_task_gnn = CrossTaskGNN(
    ...
    def get_optimizer(self):
        if self.use_gnn:
            params = (
                ...
                + list(self.cross_task_gnn.parameters())
            )
        else:
    ...
    def get_speaker_ids_2d(self, padded_speaker_masks, padded_labels_2d):
        ...
        if padded_speaker_masks.dim() == 3:
            if padded_speaker_masks.size(0) == B and padded_speaker_masks.size(1) == L:
                speaker_ids = torch.argmax(padded_speaker_masks, dim=-1)
            elif padded_speaker_masks.size(0) == L and padded_speaker_masks.size(1) == B:
                speaker_ids = torch.argmax(padded_speaker_masks, dim=-1).transpose(0, 1)
            else:
                raise ValueError(
    ...
    def get_effective_gnn_edge_mode(self):
        ...
        speaker_temporal:
            self-loop + temporal edge + same-speaker edge

        temporal:
            self-loop + temporal edge only

        speaker:
            self-loop + same-speaker edge only
        """
    ...
    def build_dialogue_speaker_temporal_adj(
        self,
        speaker_ids_2d,
        padded_labels_2d
    ):
        ...
        valid_2d = padded_labels_2d != -1
        valid_flat = valid_2d.reshape(-1)

        flat_speaker = speaker_ids_2d.reshape(-1)[valid_flat]

        dialogue_ids = torch.arange(
            B,
            device=device
        ).unsqueeze(1).expand(B, L)
        ...
        flat_dialogue = dialogue_ids.reshape(-1)[valid_flat]
        flat_position = position_ids.reshape(-1)[valid_flat]

        N = flat_speaker.size(0)

        if N <= 1:
            return torch.ones(1, N, N, device=device)

        same_dialogue = flat_dialogue.unsqueeze(1).eq(
            flat_dialogue.unsqueeze(0)
        )

        same_speaker = flat_speaker.unsqueeze(1).eq(
            flat_speaker.unsqueeze(0)
        )

        position_distance = torch.abs(
            flat_position.unsqueeze(1)
            - flat_position.unsqueeze(0)
        )

        temporal_edge = same_dialogue & (position_distance <= 1)
        speaker_edge = same_dialogue & same_speaker
    ...
    def train_or_eval_model_per_epoch(self, dataloader, train=True):
        if train:
            self.model.train()
            if self.use_gnn:
                self.cross_task_gnn.train()
        else:
            self.model.eval()
            if self.use_gnn:
                self.cross_task_gnn.eval()
        ...
        for _, data in enumerate(dataloader):
            ...
            if self.use_gnn:
                ...
                adj = self.build_dialogue_speaker_temporal_adj(
                    speaker_ids_2d,
                    padded_labels_2d
                )

                gnn_input = fc_outputs.unsqueeze(0)

                gnn_delta = self.cross_task_gnn(
                    gnn_input,
                    adj
                )

                refined_fc_outputs = fc_outputs + self.gnn_alpha * gnn_delta
...
def get_args():
    ...
    parser.add_option(
        ...
        default=0,
        type='int',
        help='whether to use residual GNN adapter, 1 or 0'
    )
    ...
    parser.add_option(
        ...
        default=0.1,
        type='float',
        help='residual strength of GNN adapter'
    )
    ...
    parser.add_option(
        ...
        default='auto',
        type='str',
        help='auto / speaker_temporal / temporal / speaker'
    )
